...
export default async function handler(req, res) {
  try {
    ...
    const response = await fetch(`${process.env.BACKEND_URL}/process-workflow-image`, {
      method: 'POST',
      body: buffer,
      headers: {
        ...req.headers,
    ...
    res.status(500).json({ error: 'Failed to process workflow image', details: error.message });
  }
}
